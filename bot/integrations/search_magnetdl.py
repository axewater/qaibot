# bot/integrations/search_magnetdl.py
from bs4 import BeautifulSoup
import json, sys, logging, requests

def search_torrents(search_query):
    logging.info(f"search_torrents: Starting search for: {search_query}")
    ...
    logging.info(f"search_torrents: Constructed URL: {search_url}")

    try:
        response = requests.get(search_url, headers=headers)
        response.raise_for_status()
    except requests.RequestException as e:
        logging.error(f"search_torrents: Failed to retrieve data: {e}")
        return [] 

    logging.info(f"search_torrents: Received response with status code: {response.status_code}")
    ...
    if not table:
        logging.warning("search_torrents: No results found - no table present on the page.")
        return [] 
    ...
    for row in rows:
        ...
        if len(columns) < 8:  # Check if all expected columns are present
            logging.warning("search_torrents: Skipping a row with insufficient data.")
            continue

        seeders = columns[-2].text.strip()
        if seeders == '0':
            logging.info("search_torrents: Skipping torrent with 0 seeders.")
    ...
    if not results:
        logging.info("search_torrents: No valid torrents found with seeders after processing all rows.")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python search_magnetdl.py \"<search query>\"")
        sys.exit(1)
