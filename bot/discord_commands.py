...
from .commands.sectools.exploitdb import handle_exploitdb
from .commands.ebay import handle_ebay
...
async def setup(bot):
    ...
    @bot.slash_command(name="qexploitdb", description="Search Exploit Database.")
    async def exploitdb(interaction: discord.Interaction, 
                        query: str,
                        platform: str = Option(str, default=None, required=False, description="Specific platform to search exploits for."),
                        cve: str = Option(str, default=None, required=False, description="CVE identifier to search exploits for."),
                        nb_results: int = Option(int, default=5, required=False, description="Number of results to return (default: 5).")):
        logging.info(f"ExploitDB command called with query: {query}, platform: {platform}, CVE: {cve}, number of results: {nb_results}")
        await handle_exploitdb(interaction, query, platform, cve, nb_results)
    ...
    @bot.slash_command(name="qebay", description="Search for items on eBay.")
    async def ebay(interaction: discord.Interaction, 
                   query: str,
                   max_results: int = Option(int, default=10, required=False, description="Maximum number of results to return (default: 10).")):
        logging.info(f"eBay command called with query: {query}, max results: {max_results}")
        await handle_ebay(interaction, query, max_results)
